...
- `CUP_SOCKET` - Socket
- `CUP_THEME` - Theme
- `CUP_IMAGES_EXCLUDE` - Images to exclude from checks, comma-separated.
- `CUP_IMAGES_EXTRA` - Extra images to check, comma-separated.
- `CUP_REGISTRIES_AUTHENTICATION` - Registry authentication codes, comma-separated REGISTRY:AUTHENTICATION format (eg. `REGISTRY1:AUTHENTICATION1,REGISTRY2:AUTHENTICATION2`).
- `CUP_REGISTRIES_INSECURE` - Registries that can be accessed insecurely, comma-separated.
- `CUP_REGISTRIES_IGNORE` - Registries to ignore, comma-separated.
